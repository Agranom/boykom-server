...
export class FamilyGroupService {
  constructor(
    @InjectModel('familyGroup') private familyGroup: Model<FamilyGroupDocument>,
  ...
  async getByUserId(id: string): Promise<FamilyGroup | null> {
    const group: FamilyGroupDocument | null = await this.familyGroup.findOne({
  ...
  async addMembers(
    groupId: string,
    { userIds }: UpdateGroupDto,
  ): Promise<FamilyGroup | IStatusResponse> {
    if (await this.isMemberExistInGroups(userIds)) {
      return { success: false, message: staticText.familyGroup.memberAlreadyExists };
    }
    const newMembers = userIds.map((id) => ({ userId: id }));
    const updatedGroup = await this.familyGroup.findByIdAndUpdate(
      groupId,
  ...
  async removeMember(groupId: string, memberId: string): Promise<{ memberId: string }> {
    const group: FamilyGroupDocument | null = await this.familyGroup.findOneAndUpdate(
      { $and: [{ _id: groupId }, { 'members.userId': memberId }] },
  ...
  async acceptMembership(groupId: string, user: IUserRequestPayload): Promise<FamilyGroup> {
    const group: FamilyGroupDocument | null = await this.familyGroup.findOneAndUpdate(
      { $and: [{ _id: groupId }, { 'members.userId': user.userId }] },
  ...
  private async isMemberExistInGroups(memberIds: string[]): Promise<boolean> {
    const isMemberExistInGroups = await this.familyGroup.exists({
      $or: [{ 'members.userId': { $in: memberIds } }, { ownerId: { $in: memberIds } }],
    });
