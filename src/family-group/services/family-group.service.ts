...
export class FamilyGroupService {
  ...
  async acceptMembership(groupId: string, memberId: string): Promise<FamilyGroup> {
    const group: FamilyGroupDocument | null = await this.familyGroup.findOneAndUpdate({ $and: [{ _id: groupId }, { 'members.userId': memberId }] }, { $set: { 'members.$.isAccepted': true } }, { new: true });
    if (!group) {
      throw new NotFoundException('Group was not found');
    }
    return group.toJSON();
